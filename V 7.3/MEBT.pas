...
type
	...
	Tplayer = record
		npsx,npsy,npsz,psx,psy,pl,pd,pu,px,py,ph,pz:integer;
...
function check(x,y,r,b:integer):boolean;
var i,j:integer;
	o:boolean;
	begin
	o:=false;
	for i:=x-r to x+r do
		for j:=y-r to y+r do
			if (i <= MAXX-r) and (i >= MINX+r) and (j <= MAXY-r) and (j >= MINY+r) then
				begin
				if map[i,j,play.pz] = b then
					o:=true;
				end;
	check:=o;
	end;
...
if k = true then
	...
	j:=0;
	for i:=1 to 10 do
		if (Spear[i].E = false) and (j = 0) then
			begin
			Spear[i].E:=true;
			Spear[i].x:=play.px;
			Spear[i].y:=play.py;
...
if k = false then
	...
	for i:=1 to 10 do
		begin
		if (Spear[i].E = true) and (Spear[i].c < 3) then
			Spear[i].c:=Spear[i].c+1;
		...
		if (Spear[i].E = true) and (Spear[i].c = 3) then
			...
			if not (map[x,y,play.pz] = 1) then 
				begin
				Spear[i].E := false;
				end;
			
			
			if (map[x,y,play.pz] = 1) and (Spear[i].E = true) then
				begin
				for j:=1 to MAXE do
					if (enemyarr[j].enx = Spear[i].x) and (enemyarr[j].eny = Spear[i].y) then
						begin
						enemyarr[j].alert:=true;
						Spear[i].E := false;
						end;
						
				block(Spear[i].x,Spear[i].y);
				
				gotoxy(x,y);
				textcolor(6);
				if (Spear[i].d = 1) or (Spear[i].d = 2) then
					write(chr(196));
				if (Spear[i].d = 3) or (Spear[i].d = 4) then
					write(chr(179));
			...
			if (Spear[i].E = false) then
				begin
				j:=random(5)+1;
				if j < 5 then
					map[Spear[i].x,Spear[i].y,play.pz]:=23;
				if j = 5 then
					map[Spear[i].x,Spear[i].y,play.pz]:=34;
				block(Spear[i].x,Spear[i].y);
				end;
			end;
		end;	
							...
							if play.inv[i] = 12 then
								chesplace(i)
										...
										bed(false,i)
											...
											if not (play.inv[i] = 14) and not (play.inv[i] = 18) and not (play.inv[i] = 30) and not (play.inv[i] = 23) then
											begin
...
if (play.inv[a] = 34) and (play.inv[b] = 34) or (play.inv[a] = 34) and (play.inv[b] = 34) then
		begin
		play.inv[b]:=1;
		play.inv[a]:=35;
		sve:=1;
...
redraw(TRUE,1,play.py-1,MAXX,play.py);
play.pl := 10;
end;
...
var ch,ch1:char;
	...
	if keypressed() then
		...
		case ch of	{if you want a door add it in here as one \/}
			#0: begin ch1:=readkey;
				...
				#75 : begin  play.pd := 1; if (map[x-1,y,play.pz] = 1)  then play.px := play.px-1 else
						if (map[x-1,y,play.pz] = 7) then play.px := play.px-1 else if (map[x-1,y,play.pz] = 14) or  (map[x-1,y,play.pz] = 15) then play.px := play.px-1
						else if (map[x-1,y,play.pz] = 24) or (map[x-1,y,play.pz] = 25) then play.px := play.px-1; redraw(false,1,1,1,1); end;
				...
				#72 : begin  play.pd := 3; if (map[x,y-1,play.pz] = 1)  then play.py := play.py-1 else
						if (map[x,y-1,play.pz] = 7) then play.py := play.py-1 else if (map[x,y-1,play.pz] = 14) or  (map[x,y-1,play.pz] = 15) then play.py := play.py-1
						else if (map[x,y-1,play.pz] = 24) or (map[x,y-1,play.pz] = 25) then play.py := play.py-1; redraw(false,1,1,1,1); end;
		...
		if (play.px <= MAXX-1) and (play.px >= MINX+1) and (play.py <= MAXY-1) and (play.py >= MINY+1) then
			begin
			textcolor(9);
			textbackground(0);
			gotoxy(play.px,play.py);
		...
		begin		
			if enemyarr[i].enm >= 10 then
				begin
					...
					if not (x = enemyarr[i].enx) or not (y = enemyarr[i].eny) then
						begin
				...
				if IOResult = 0 then
					begin
						...
						if not (txtfile = '') then
							...
							delay(500);
							WriteCenter(40,12,'Press Enter '+chr(27)+chr(217));
							readkey;
							end;
